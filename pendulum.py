...
plt.plot(t, solution[:,0], color = 'r', linestyle = '-',linewidth = 2,label =r'$ \frac{d\theta_1}{dt}$ = $\theta_2$')
plt.plot(t, solution[:,1], color = 'b', linestyle ='--',linewidth = 1.5,label = r'$ \frac{d\theta_2}{dt}$ = $-\frac{b}{m}\theta_2 - \frac{g}{l}sin\theta_1 $')
plt.xlabel('time(s)')
plt.ylabel('plot')
plt.grid(True)
...
direc = os.getcwd()
print(direc)
img_folder = direc +'/imgs'
nfiles = len(os.listdir(img_folder))
new_folder = '/imgs/{}{}{}'.format(x0,y0,nfiles)
os.makedirs(direc+'/'+new_folder)
for i in t:
    ...
    for s in sol_dis:
        ...
        filename = direc+'/'+new_folder+'/%05d.' % count
